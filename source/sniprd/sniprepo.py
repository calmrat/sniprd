...
from sniprd.utils import log, info, Date, EMAIL_REGEXP
...
class SnipRepoSQLAlchemy(SnipRepo):
    def __init__(self, *args, **kwargs):
        super(SnipRepoSQLAlchemy, self).__init__(*args, **kwargs)
        self.engine = sqla.create_engine(self.uri, echo=self.debug)
        self.Session = sqla.orm.sessionmaker(bind=self.engine)

    def write(self):
        super(SnipRepoSQLAlchemy, self).write()
